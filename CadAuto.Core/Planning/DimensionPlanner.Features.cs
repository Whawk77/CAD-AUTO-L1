...
public sealed partial class DimensionPlanner
{
	...
	private void AddSlotDimensions(DimensionPlan plan, OutlineFeature2D outline, Datum2D datum, IEnumerable<HoleFeature2D> holes, IEnumerable<SlotFeature2D> slots)
	{
		...
		{
			AddSlotCenterDistanceDimension(plan, outline, item);
		}
		if (!flag)
		{
	...
	private void AddVerticalSlotContinuousDimensions(DimensionPlan plan, OutlineFeature2D outline, Datum2D datum, IList<SlotFeature2D> slots)
	{
		...
		foreach (List<Point2D> item in GroupSlotAnchorsByCoordinate(slots, datum, (Point2D p) => p.Y))
		{
			...
			if (list.Count > 0)
			{
				...
				Point2D target = side == DimensionSide.Right
	...
	private void AddHorizontalSlotContinuousDimensions(DimensionPlan plan, OutlineFeature2D outline, Datum2D datum, IList<SlotFeature2D> slots)
	{
		...
		foreach (List<Point2D> item in GroupSlotAnchorsByCoordinate(slots, datum, (Point2D p) => p.X))
		{
			...
			if (list.Count != 0)
			{
				...
				Point2D point2D = side == DimensionSide.Top
					? list.OrderByDescending(point => point.Y).First()
					: list.OrderBy(point => point.Y).First();
				...
				else if (slotFeature2D != null)
				{
					if (AddHorizontalOutlineReferenceDimension(plan, outline, datum.BaseX, point2D, DimensionKind.Normal, side, string.Empty, "DoubleArcSlotHorizontalDatum", slotFeature2D.GroupId, skippedDebugRole: "SlotDatumH"))
					{
						LinkSlotCenterToDatumSide(plan, slotFeature2D, list.Select(point => FindSlotByAnchor(slots, point)).Where(slot => slot != null), side);
					}
				}
				else
				{
	...
	private void AddSlotCenterDistanceDimension(DimensionPlan plan, OutlineFeature2D outline, SlotFeature2D slot)
	{
		...
		if ((num <= _config.GeometryTolerance && num2 <= _config.GeometryTolerance)
			|| slot.Radius <= _config.GeometryTolerance
			|| !OutlineGeometryQuery.TryGetEnvelope(outline, _config.GeometryTolerance, out var envelope))
		{
			return;
		}
		if (num >= num2)
		{
			double bottomDistance = Math.Min(firstCenter.Y, secondCenter.Y) - slot.Radius - envelope.MinY;
			double topDistance = envelope.MaxY - Math.Max(firstCenter.Y, secondCenter.Y) - slot.Radius;
			DimensionSide side = topDistance + _config.GeometryTolerance < bottomDistance ? DimensionSide.Top : DimensionSide.Bottom;
			double offset = side == DimensionSide.Top ? slot.Radius : -slot.Radius;
			AddDimension(plan, DimensionKind.Normal, DimensionOrientation.Horizontal, side, new Point2D(firstCenter.X, firstCenter.Y + offset), new Point2D(secondCenter.X, secondCenter.Y + offset), string.Empty, "SlotCenter", slot.GroupId, preservePreferredSide: true);
		}
		else
		{
			double leftDistance = Math.Min(firstCenter.X, secondCenter.X) - slot.Radius - envelope.MinX;
			double rightDistance = envelope.MaxX - Math.Max(firstCenter.X, secondCenter.X) - slot.Radius;
			DimensionSide side = rightDistance + _config.GeometryTolerance < leftDistance ? DimensionSide.Right : DimensionSide.Left;
			double offset = side == DimensionSide.Right ? slot.Radius : -slot.Radius;
			AddDimension(plan, DimensionKind.Normal, DimensionOrientation.Vertical, side, new Point2D(firstCenter.X + offset, firstCenter.Y), new Point2D(secondCenter.X + offset, secondCenter.Y), string.Empty, "SlotCenter", slot.GroupId, preservePreferredSide: true);
		}
	}

	private DimensionSide ChooseHorizontalSlotGroupSide(OutlineFeature2D outline, IList<Point2D> points)
	{
	...
	private void LinkSlotCenterToDatumSide(DimensionPlan plan, SlotFeature2D representative, IEnumerable<SlotFeature2D> slots, DimensionSide side)
	{
		PlannedDimension datum = plan.Dimensions.LastOrDefault(dimension => dimension.DebugOwner == representative.GroupId
			&& (dimension.DebugRole == "DoubleArcSlotHorizontalDatum" || dimension.DebugRole == "DoubleArcSlotVerticalDatum"));
		PlannedDimension linked = plan.Dimensions.FirstOrDefault(dimension => dimension.DebugRole == "SlotCenter"
			&& dimension.DebugOwner == representative.GroupId);
		if (datum == null || linked == null)
		{
			return;
		}
		string alignmentKey = "SlotDatum:" + representative.GroupId + ":" + datum.Orientation;
		datum.AlignmentKey = alignmentKey;
		datum.AlignmentPriority = 100;
		linked.Side = side;
		linked.RuleId = "LinkedSlotDatumSide";
		linked.AlignmentKey = alignmentKey;
		linked.AlignmentPriority = 90;
		linked.PreservePreferredSide = true;
		foreach (SlotFeature2D slot in slots.Where(slot => slot != null && slot.GroupId != representative.GroupId
			&& slot.IsVertical == representative.IsVertical
			&& Math.Abs(slot.CenterDistance - representative.CenterDistance) <= _config.GeometryTolerance))
		{
			PlannedDimension duplicate = plan.Dimensions.FirstOrDefault(dimension => dimension.DebugRole == "SlotCenter"
				&& dimension.DebugOwner == slot.GroupId);
			if (duplicate != null)
			{
				plan.MarkSuppressed(duplicate, SuppressReason.MirroredDuplicate);
				plan.Dimensions.Remove(duplicate);
			}
		}
	}
	...
	private void AddNonPinHolesFromOutlineEdge(DimensionPlan plan, OutlineFeature2D outline, Datum2D datum, IList<HoleFeature2D> holes)
	{
		...
		if (baseHole != null)
		{
			...
			DimensionSide horizontalSide = ChooseHorizontalHoleSide(outline, baseHole.Center);
			DimensionSide verticalSide = ChooseVerticalHoleSide(outline, baseHole.Center);
			string horizontalAlignmentKey = GetNonPinChainAlignmentKey(baseHole, normalTargets, horizontalSide, horizontal: true);
			string verticalAlignmentKey = GetNonPinChainAlignmentKey(baseHole, normalTargets, verticalSide, horizontal: false);
			AddNonPinHoleDatumLocation(plan, outline, datum, baseHole, horizontalSide, verticalSide, horizontalAlignmentKey, verticalAlignmentKey);
			AddNonPinHoleChainsFromBase(plan, baseHole, normalTargets, "HoleChain", horizontalSide, verticalSide, horizontalAlignmentKey, verticalAlignmentKey);
	...
	private void AddNonPinHoleChainsFromBase(DimensionPlan plan, HoleFeature2D baseHole, IList<HoleFeature2D> targets, string debugRole, DimensionSide horizontalSide, DimensionSide verticalSide, string horizontalAlignmentKey, string verticalAlignmentKey)
	{
		if (targets != null && targets.Count != 0)
		{
			List<HoleFeature2D> list = new List<HoleFeature2D> { baseHole };
			list.AddRange(targets);
			AddNonPinHoleAxisChain(plan, list, baseHole, horizontalAxis: true, debugRole + "H", horizontalSide, horizontalAlignmentKey);
			List<HoleFeature2D> list2 = new List<HoleFeature2D> { baseHole };
			list2.AddRange(targets);
			AddNonPinHoleAxisChain(plan, list2, baseHole, horizontalAxis: false, debugRole + "V", verticalSide, verticalAlignmentKey);
		}
	}
	...
	{
		DimensionSide horizontalSide = ChooseLooseDimensionSide(outline, new[] { baseHole }, horizontal: true);
		DimensionSide verticalSide = ChooseLooseDimensionSide(outline, new[] { baseHole }, horizontal: false);
		AddNonPinHoleChainsFromBase(plan, baseHole, targets, debugRole, horizontalSide, verticalSide, null, null);
	}

	private void AddNonPinHoleAxisChain(DimensionPlan plan, IList<HoleFeature2D> holes, HoleFeature2D anchorHole, bool horizontalAxis, string debugRole, DimensionSide side, string alignmentKey)
	{
		Func<HoleFeature2D, double> axisCoordinate = h => horizontalAxis ? h.Center.X : h.Center.Y;
		Func<HoleFeature2D, double> perpendicularCoordinate = h => horizontalAxis ? h.Center.Y : h.Center.X;
		Func<HoleFeature2D, double> sideDistance = h => horizontalAxis
			? (side == DimensionSide.Bottom ? h.Center.Y : -h.Center.Y)
			: (side == DimensionSide.Left ? h.Center.X : -h.Center.X);
		double tolerance = GetFunctionalHoleAlignmentTolerance();
		List<HoleFeature2D> list = ClusterCoordinates(holes.Select(axisCoordinate))
			.Select(coordinate => holes.Where(h => Math.Abs(axisCoordinate(h) - coordinate) <= tolerance)
				.OrderBy(h => Math.Abs(perpendicularCoordinate(h) - perpendicularCoordinate(anchorHole)))
				.ThenBy(sideDistance).First())
			.OrderBy(axisCoordinate)
			.ToList();
		for (int num = 1; num < list.Count; num++)
		{
			AddDimension(plan, DimensionKind.HoleLocation, (!horizontalAxis) ? DimensionOrientation.Vertical : DimensionOrientation.Horizontal, side, list[num - 1].Center, list[num].Center, string.Empty, debugRole, alignmentKey: alignmentKey, alignmentPriority: string.IsNullOrEmpty(alignmentKey) ? 0 : 90);
		}
	}

	private string GetNonPinChainAlignmentKey(HoleFeature2D baseHole, IList<HoleFeature2D> targets, DimensionSide side, bool horizontal)
	{
		...
		HoleFeature2D first = chain.OrderBy(h => horizontal ? h.Center.X : h.Center.Y).First();
		HoleFeature2D last = chain.OrderByDescending(h => horizontal ? h.Center.X : h.Center.Y).First();
		return "LooseHoleChain:" + side.ToString() + ":" + (horizontal ? "H" : "V") + ":"
	...
	private void AddNonPinHoleDatumLocation(DimensionPlan plan, OutlineFeature2D outline, Datum2D datum, HoleFeature2D hole, DimensionSide horizontalSide, DimensionSide verticalSide, string horizontalAlignmentKey, string verticalAlignmentKey)
	{
		AddHorizontalOutlineReferenceDimension(plan, outline, datum.BaseX, hole.Center, DimensionKind.HoleLocation, horizontalSide, string.Empty, "HoleDatumX", alignmentKey: horizontalAlignmentKey, alignmentPriority: string.IsNullOrEmpty(horizontalAlignmentKey) ? 0 : 100);
		AddVerticalOutlineReferenceDimension(plan, outline, datum.BaseY, hole.Center, DimensionKind.HoleLocation, verticalSide, string.Empty, "HoleDatumY", alignmentKey: verticalAlignmentKey, alignmentPriority: string.IsNullOrEmpty(verticalAlignmentKey) ? 0 : 100, preservePreferredSide: true);
	}
	...
	{
		return slots?.FirstOrDefault((SlotFeature2D s) => s != null
			&& (PointsEqual(s.FirstCenter, anchor) || PointsEqual(s.SecondCenter, anchor)));
	}

	private static Point2D GetDoubleArcSlotCenterFacingSide(SlotFeature2D slot, Point2D fallback, DimensionSide side)
	{
		if (slot == null || slot.IsSingleArcSlot)
		{
			return fallback;
		}
		...
		{
			return side == DimensionSide.Left
				? (slot.FirstCenter.X <= slot.SecondCenter.X ? slot.FirstCenter : slot.SecondCenter)
				: (slot.FirstCenter.X >= slot.SecondCenter.X ? slot.FirstCenter : slot.SecondCenter);
		}
		return side == DimensionSide.Bottom
			? (slot.FirstCenter.Y <= slot.SecondCenter.Y ? slot.FirstCenter : slot.SecondCenter)
			: (slot.FirstCenter.Y >= slot.SecondCenter.Y ? slot.FirstCenter : slot.SecondCenter);
	}
	...
	{
		DimensionSide side = ChooseHorizontalHoleSide(outline, center);
		...
		{
			plan.AddSkippedDimension(DimensionKind.Normal, DimensionOrientation.Horizontal, side, new Point2D(datum.BaseX, center.Y), center, "NoRealOutlineAttachment:XDatum=" + datum.BaseX.ToString("0.########", CultureInfo.InvariantCulture), "SingleArcSlotDatum", slot.GroupId);
			return;
		}
		firstPoint = GetSideFacingOutlineSegmentEndpoint(outline, firstPoint, DimensionOrientation.Horizontal, side);
		Point2D singleArcSlotHorizontalGripPoint = GetSingleArcSlotHorizontalGripPoint(slot, side);
		double num = Math.Abs(center.X - datum.BaseX);
		if (!(num <= _config.GeometryTolerance))
		{
			AddDimension(plan, DimensionKind.Normal, DimensionOrientation.Horizontal, side, firstPoint, singleArcSlotHorizontalGripPoint, _config.FormatNumber(num), "SingleArcSlotDatum", slot.GroupId);
		}
	}

	private static Point2D GetSingleArcSlotHorizontalGripPoint(SlotFeature2D slot, DimensionSide side)
	{
		double offset = side == DimensionSide.Top ? slot.Radius : -slot.Radius;
		return new Point2D(slot.FirstCenter.X, slot.FirstCenter.Y + offset);
	}
