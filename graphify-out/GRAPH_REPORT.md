# Graph Report - 8.25  (2026-08-28)

## Corpus Check
- 145 files · ~194,173 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2330 nodes · 8257 edges · 111 communities (100 shown, 11 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 623 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7cd74d9b`
...
1. `OutlineFeature2D` - 421 edges
2. `Point2D` - 254 edges
3. `Program` - 238 edges
4. `PlannedDimension` - 173 edges
5. `DimensionPlan` - 151 edges
6. `DimensionSide` - 127 edges
...
Cohesion: 0.12
Nodes (9): HoleFeature2D, DimensionPlanner, IEnumerable, IList, List, Tuple, FunctionalHoleGroupPlan, LooseHoleLocationPlan (+1 more)
...
Cohesion: 0.14
Nodes (7): LProfileAxisChain, LProfileMatch, LProfileOuterContour, IEnumerable, LProfileArcAttachment, LProfileAxisChain, LProfileConcaveTurn

### Community 6 - "OutlineSegment"
Cohesion: 0.12
Nodes (4): Point2D, FilletFeature2D, StructureEndpointSpan, IEquatable
...
Cohesion: 0.08
Nodes (11): CoordinateFrameModelTransform, HoleCalloutCluster, List, SlotArcCandidate, SlotLineCandidate, CadAuto.Core.Recognition, CadAuto.Core.Geometry, CadAuto.Core.Model (+3 more)
...
Cohesion: 0.05
Nodes (4): Program, Action, HashSet, int
...
Cohesion: 0.13
Nodes (15): LProfileArcAttachment, LProfileBoundaryGraph, LProfileConcaveTurn, LProfileEdge, LProfileFilletProjection, LProfileHalfEdge, LProfileLocalAnchorCandidate, LProfileLocalAnchorPair (+7 more)
...
Cohesion: 0.17
Nodes (11): AnnotationCase, AnnotationCaseDecision, AnnotationCaseRuntime, AnnotationStrategyIds, IList, List, string, AnnotationCaseOverlay (+3 more)
...
Cohesion: 0.23
Nodes (9): DiagnosticDimensionSide, Commands, IEnumerable, int, string, StringBuilder, DiagnosticRunContext, Exception (+1 more)
...
Cohesion: 0.27
Nodes (5): IList, LProfileBoundaryGraph, LProfileFilletProjection, LProfileLocalAnchorCandidate, LProfileOverallSplitEvidence
...
Cohesion: 0.15
Nodes (8): OutlineFeature2D, List, IEnumerable, DimensionPlanner, IgnoredPoint, IList, List, StructurePoint

### Community 63 - "DimensionExtensionLineRenderer"
Cohesion: 0.21
Nodes (4): CoordinateFrame2D, double, IEnumerable, List

### Community 64 - ".AssertLProfileRuleEndpointsOnBoundary"
Cohesion: 0.22
Nodes (4): IEnumerable, IList, ISet, List
...
- **Why does `OutlineFeature2D` connect `.ArbitrateOuterContourStepOverallRemainders` to `Point2D Module`, `OutlineFeature2D Module`, `FeatureRecognizer2D Module`, `OutlineSegment`, `Assert Module`, `Segment2D`, `DimensionDrawer Module`, `DimensionPlanner`, `FeatureRecognizer2D`, `FeatureRecognizer Module`, `CadAuto Core Geometry`, `OutlineSegment Module`, `.CreateDefault`, `DimensionPlanner`, `FeatureRecognizer2D.cs`, `StackingLayerItem Module`, `.HorizontalProbeCrossesInterior`, `DimensionLayoutRules.cs`, `HoleFeature2D`, `.VerticalExtensionOverlapsOutlineSegment`, `HoleCalloutPlanner`, `DimensionDrawer Module`, `SlotFeature2D`, `FeatureRecognizer`, `.IsVertical`, `AnnotationCaseStore`, `.PointsEqual`, `.SuppressOutlineSegmentsOnOverallEnvelope`, `.PromptForDatumHole`, `DimensionExtensionLineRenderer`, `.AssertLProfileRuleEndpointsOnBoundary`, `FeatureRecognizer.cs`, `.SuppressStructureClosedChainRedundantPositioningOnSide`, `.CreatePartitionItem`, `.Place`, `.AssertTest2BottomContourPlan`, `.OrthogonalRotatedVerticalChainKeepsRealWidths`?**
  _High betweenness centrality (0.244) - this node is a cross-community bridge._
- **Why does `DimensionDrawer` connect `OutlineSegment Module` to `GeometryCollector Module`, `DimensionDrawer Module`, `HoleCalloutPlan Module`, `OutlineGeometryQuery`, `StackingLayerItem Module`, `.VerticalExtensionOverlapsOutlineSegment`, `NativeDiameterDimensioner Module`, `DimensionRuleConfig`, `SlotArcCandidate Module`, `FeatureRecognizer`, `.AppendDimensionDiagnostics`, `DimensionRuleConfig`, `CoordinateFrame2D`, `.ArbitrateOuterContourStepOverallRemainders`, `DimensionExtensionLineRenderer`, `PluginEntry`, `FeatureRecognizer2D.cs`, `DebugAnnotationRenderer Module`, `DimensionExtensionLineRenderer`?**
  _High betweenness centrality (0.125) - this node is a cross-community bridge._
- **Why does `Point2D` connect `OutlineSegment` to `Point2D Module`, `OutlineFeature2D Module`, `FeatureRecognizer2D Module`, `Assert Module`, `Segment2D`, `DimensionDrawer Module`, `DimensionPlanner`, `FeatureRecognizer2D`, `CadAuto Core Geometry`, `.CreateDefault`, `HoleCalloutPlan Module`, `FeatureRecognizer2D.cs`, `StackingLayerItem Module`, `.HorizontalProbeCrossesInterior`, `DimensionLayoutRules.cs`, `HoleFeature2D`, `.VerticalExtensionOverlapsOutlineSegment`, `DimensionDrawer Module`, `SlotFeature2D`, `.IsVertical`, `.AddLooseHoleCenterDistances`, `DimSide Module`, `AnnotationCaseStore`, `.DrawSlotRadiusLeadersWithJig`, `DimensionPlanner.cs`, `.SuppressOutlineSegmentsOnOverallEnvelope`, `.ArbitrateOuterContourStepOverallRemainders`, `.PromptForDatumHole`, `DimensionExtensionLineRenderer`, `.Place`, `.OrthogonalRotatedVerticalChainKeepsRealWidths`?**
...
- **Should `Point2D Module` be split into smaller, more focused modules?**
  _Cohesion score 0.12220205686630368 - nodes in this community are weakly interconnected._
